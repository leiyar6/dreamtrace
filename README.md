...
- **数据存储**：localStorage
- **时区**：所有日期处理显式使用 `Asia/Shanghai` 时区，避免 UTC 偏差
...
- 视口固定 390×844px，单屏布局，禁止滚动
- 主题色：紫色调（`#7B68EE` / `#9B8DF8`）+ 暖色点缀（米白 / 柔桃 / 月黄）
- 所有插画统一梦幻水彩风格，无文字、无水印、无边框
- 图片资源使用 `landscape_16:9` 横图匹配卡片比例
- 状态栏与导航栏在不同页面自动切换主题色
